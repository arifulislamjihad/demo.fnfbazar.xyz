...
from .models import (
    ...
    SiteSettings, Attribute, AttributeValue, ProductVariant,
...
class ProductVariantInline(admin.TabularInline):
    ...
    extra = 0
    fields = ("attribute_values", "price", "stock", "sku", "image", "is_active")
...
class OrderAdmin(admin.ModelAdmin):
    list_display = ("order_id_display", "customer_info_display", "order_items_display", "amount_info_display", "payment_status_display", "status_label", "steadfast_info", "created_at_display")
    list_filter = ("status", "paid", "payment_method", "created", "steadfast_status", "vendor")
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [path('add-quick-note/<int:order_id>/', self.admin_site.admin_view(self.process_add_note), name='order-add-quick-note')]
    ...
    def process_add_note(self, request, order_id):
        if request.method == 'POST':
            ...
            if note_text:
                ...
                OrderNote.objects.create(order=order, user=request.user, note=note_text)
                messages.success(request, f"Note added to Order #{order_id}")
            else: messages.warning(request, "Note cannot be empty.")
    ...
    def send_to_steadfast_action(self, request, queryset):
        ...
        if success_count > 0: messages.success(request, f"{success_count} orders sent to Steadfast successfully.")
    ...
    def amount_info_display(self, obj): return format_html('<div style="font-size:14px; font-weight:bold; color:#108a00;">৳{}</div><div style="font-size:10px; color:#666;">Delivery: ৳{}</div>', obj.get_total_cost(), obj.delivery_charge)
    amount_info_display.short_description = "Amount"
    ...
    def customer_info_display(self, obj):
        ...
        script = f"""<script>function promptNote_{obj.id}() {{ let note = prompt("Enter note for Order #{obj.id}:"); if (note != null && note != "") {{ let form = document.createElement('form'); form.method = 'POST'; form.action = '{add_note_url}'; let csrf = document.querySelector('[name=csrfmiddlewaretoken]').value; let csrfInput = document.createElement('input'); csrfInput.type = 'hidden'; csrfInput.name = 'csrfmiddlewaretoken'; csrfInput.value = csrf; form.appendChild(csrfInput); let noteInput = document.createElement('input'); noteInput.type = 'hidden'; noteInput.name = 'note'; noteInput.value = note; form.appendChild(noteInput); document.body.appendChild(form); form.submit(); }} }}</script>"""
    ...
    def fraud_check_badge(self, obj):
        ...
        for courier_name, info in courier_data.items():
            ...
            if stats:
                ...
                if c_total > 0:
                    has_data = True
                    popup_rows += f"""<tr style="border-bottom: 1px solid #eee;"><td style="padding:4px; text-transform:capitalize; color:#000;">{courier_name}</td><td style="padding:4px; text-align:center; color:#000;">{c_total}</td><td style="padding:4px; text-align:center; color:green; font-weight:bold;">{c_success}</td><td style="padding:4px; text-align:center; color:red; font-weight:bold;">{c_cancel}</td></tr>"""
        if not has_data: popup_rows = "<tr><td colspan='4' style='padding:5px; text-align:center;'>No details.</td></tr>"
        if cancel_rate > 30: badge = f'<div style="background:#ffebee; color:#c62828; padding:2px 6px; border-radius:10px; border:1px solid #c62828; font-weight:bold; font-size:10px; cursor:pointer; display:inline-block;">⚠️ Risky ({int(cancel_rate)}%)</div>'
        else: badge = f'<div style="background:#e8f5e9; color:#2e7d32; padding:2px 6px; border-radius:10px; border:1px solid #2e7d32; font-weight:bold; font-size:10px; cursor:pointer; display:inline-block;">✅ Safe ({int(success_rate)}%)</div>'
        html = f"""<style>.fraud-wrapper {{ position: relative; display: inline-block; }} .fraud-wrapper .fraud-popup {{ visibility: hidden; opacity: 0; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 999999; width: 300px; background: #fff; border-radius: 8px; box-shadow: 0 0 0 100vw rgba(0,0,0,0.5), 0 10px 30px rgba(0,0,0,0.5); border: 1px solid #ccc; transition: 0.2s; }} .fraud-wrapper:hover .fraud-popup {{ visibility: visible; opacity: 1; }} .popup-table {{ width: 100%; border-collapse: collapse; font-size:11px; }} .popup-table th {{ background: #f3f4f6; padding: 5px; text-align: center; }}</style><div class="fraud-wrapper">{badge}<div class="fraud-popup"><div style="background:#333; color:#fff; padding:8px; border-radius:6px 6px 0 0; display:flex; text-align:center;"><div style="flex:1;">Total: {total}</div><div style="flex:1; color:#4ade80;">Success: {success}</div><div style="flex:1; color:#f87171;">Cancel: {canceled}</div></div><table class="popup-table"><thead><tr><th style="text-align:left;">Courier</th><th>Total</th><th>Ok</th><th>X</th></tr></thead><tbody>{popup_rows}</tbody></table></div></div>"""
...
class VendorWebhookAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('webhook_link_display', 'user') 
    
    fieldsets = (
        ("Integration Details", {
            'fields': ('user', 'webhook_link_display', 'is_active'),
            'description': "Copy the link below and paste it into your WordPress WooCommerce Webhook settings."
        }),
    )
    ...
    def webhook_link_display(self, obj):
        domain = "https://your-domain.com" 
        if settings.DEBUG:
            domain = "http://127.0.0.1:8000"
            
        url = f"{domain}/api/webhook/woocommerce-order/{obj.user.id}/"
        return mark_safe(f'<code style="font-size: 14px; padding: 10px; background: #e0f2f1; border: 1px solid #004d40; color: #004d40; display:block; border-radius: 4px;">{url}</code>')