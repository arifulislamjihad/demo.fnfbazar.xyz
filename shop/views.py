...
from .utils import (
    ...
    send_facebook_purchase_event,
    send_facebook_lead_event,
)
...
def home(request):
    ...
    if not featured_categories:
        featured_categories = Category.objects.all()[:14]
            ...
            for val in variant.attribute_values.all():
                ...
                if val.id not in existing_ids:
                    if variant_image_url:
                        val.image_src = variant_image_url
        ...
        for key, qty in cart.items():
            try:
                if ":" in key:
                    ...
                    v = ProductVariant.objects.get(id=int(vid), product=p, is_active=True)
                    unit_price = v.get_price()
                else:
...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        if x_forwarded_for:
            order.ip_address = x_forwarded_for.split(",")[0]
        else:
